/* Screenshot each demo site's home page into assets/shots/<slug>.jpg for the hub cards.
   ...
   Requires a static server serving the repo root (e.g. python3 -m http.server 8099). */
const { chromium } = require('/Users/dissu/Documents/PP/labs/node_modules/playwright');
const fs = require('fs');
const path = require('path');

const ROOT = '/Users/dissu/Documents/PP/labs/pranny-repo/the-rep-studio';
...
(async () => {
  ...
  for (const p of list) {
    ...
    try {
      ...
      results.fail.push(p.slug + ' (' + e.message.split('\n')[0] + ')');
      console.log('shot  ✗', p.slug, '-', e.message.split('\n')[0]);
    }
    await page.close();
  }
  await browser.close();
